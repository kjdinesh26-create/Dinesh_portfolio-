...
const projects = [
  {
    ...
    description: "AN INTELLIGENT CONVERSATIONAL AGENT BUILT WITH NLP TECHNIQUES AND DEEP LEARNING ARCHITECTURES.",
    year: "2024",
  ...
  },
  {
    ...
    description: "A MINIMALIST EDITORIAL PORTFOLIO BUILT WITH NEXT.JS, GSAP ANIMATIONS, AND LIQUID MOTION.",
    year: "2024",
  ...
  },
  {
    ...
    description: "A HIGH-PERFORMANCE FINANCIAL TRACKER WITH REAL-TIME ANALYTICS AND SECURE AUTHENTICATION.",
    year: "2024",
    image: "/project_thumbnail_1_1777914174106.png",
  ...
  },
  {
    ...
    description: "A NEXT-GEN VOICE RECOGNITION SYSTEM INTEGRATED WITH LARGE LANGUAGE MODELS FOR NATURAL INTERACTION.",
    year: "2025",
    image: "/project_thumbnail_2_1777914187316.png",
...
export default function Projects() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      cards.forEach((card: any, i: number) => {
        // We only animate cards that have a card below them
        if (i !== cards.length - 1) {
          gsap.to(card, {
            scale: 0.85,
            xPercent: -5,
      ...
      // Atmospheric Background Label Animation
  ...
  return (
    <div ref={containerRef} id="projects" className="bg-bg-primary relative">
      ...
      {/* SECTION HEADER - Now relative so it scrolls away */}
      <div className="relative pt-12 pb-12 left-12 z-50 mix-blend-difference pointer-events-none px-12 flex items-center gap-6 stack-label">
        <div className="w-10 h-10 rounded-full border border-white flex items-center justify-center text-white text-xs font-bold">03</div>
        <span className="text-white text-[10px] font-bold tracking-[0.6em] uppercase">SELECTED WORKS</span>
      </div>
      ...
      <div className="flex flex-col">
          ...
          >
            <div className="w-[90vw] h-[85vh] bg-text-primary rounded-[3rem] p-12 md:p-20 relative overflow-hidden flex flex-col justify-between shadow-2xl border border-white/5">
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div className="space-y-4">
                  <div className="flex items-center gap-3 text-accent">
                    <Sparkles size={16} />
                    <span className="text-xs font-bold tracking-[0.4em] uppercase">{project.category}</span>
                  </div>
                ...
                <div className="flex gap-4">
                  <a href="#" className="w-14 h-14 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-accent hover:border-accent hover:text-text-primary transition-all">
                ...
                <div className="col-span-12 lg:col-span-5 space-y-8">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-5 py-2 border border-white/10 text-[10px] font-bold text-white/40 tracking-[0.2em] uppercase rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              <div className="absolute bottom-12 right-20 pointer-events-none">
                <span className="text-[12vw] font-display text-white/5 leading-none">{project.year}</span>
              </div>
            </div>
          </section>
        ))}
        ...
        <section className="h-screen w-full flex flex-col items-center justify-center bg-bg-primary text-center p-20 relative">
          <h4 className="text-6xl md:text-8xl font-display uppercase leading-none mb-12">
            YOUR NEXT <br/> <span className="italic" style={{ WebkitTextStroke: "1px black", color: "transparent" }}>SUCCESS STORY</span>
          </h4>
          <a href="#contact" className="group relative flex items-center gap-8 py-4 px-12 border border-text-primary rounded-full hover:bg-text-primary hover:text-bg-primary transition-all duration-500 overflow-hidden">
            <span className="relative z-10 text-xl font-bold tracking-widest">START A PROJECT</span>
